...
// ============================================
const NEGOCIO_ID_POR_DEFECTO = '56cf5508-2796-4f57-a245-2884a38b71e3'; // ID de Dayis Nails
...
window.getNombreNegocio = async function() {
    ...
    return config?.nombre || 'Dayis Nails';
};
...
window.getTelefonoDuenno = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.telefono || '56541795';
};
...
window.getEmailNegocio = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.email || 'dayisfonseca82@gmil. com';
};
...
window.getMensajeBienvenida = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.mensaje_bienvenida || '¡Bienvenida a Dayis Nails!';
};
...
window.getNtfyTopic = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.ntfy_topic || 'dayisnails';
};
...
console.log('✅ config-negocio.js listo para Dayis Nails');
